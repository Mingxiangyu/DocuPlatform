...
export const usePaymentStore = defineStore('payment', () => {
  ...
  const saveToStorage = () => {
    try {
      localStorage.setItem('payment_order_history', JSON.stringify(orderHistory.value))
      localStorage.setItem('payment_purchased_articles', JSON.stringify(purchasedArticles.value))
    } catch (error) {
      console.error('Failed to save payment data to storage:', error)
    }
  }
